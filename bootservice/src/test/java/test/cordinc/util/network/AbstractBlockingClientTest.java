package test.cordinc.util.network;
...
import com.cordinc.util.network.AbstractBlockingClient;
import com.cordinc.util.network.TwoByteMessageLength;
...
public class AbstractBlockingClientTest {
	...
	private static class ReflectServer implements Runnable {
		...
		public void run() {
			try {
			    ...
			    System.out.println("ReflectServer starts running [thread:" + Thread.currentThread().getId() + "]");
	...
	public void testConnection() throws Exception {
		...
		new Thread(client).start();
		assertTrue(connected.await(10000, TimeUnit.MILLISECONDS));
		client.write("Hello".getBytes());
		assertTrue(latch.await(10000, TimeUnit.MILLISECONDS));
		System.out.println(messageHolder.get());
		System.in.read();
		//assertEquals(messageHolder.get(), "Hello");
		client.stop();
		//assertTrue(disconnected.await(10000, TimeUnit.MILLISECONDS));
	}
	...
	public void testReadOverBufferLengthMessage() throws Exception {
		...
		// the request has to make sure of the format: whole message size followed by message
		// while the message can be splitted into multiple requests
