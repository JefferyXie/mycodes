...
public abstract class AbstractBlockingClient implements Runnable {
	...
	private final IMessageLength messageLength;
     ...
     */
	public AbstractBlockingClient(InetAddress server, int port, IMessageLength messageLength, int defaultBufferSize) {
		this.server = server;
		this.port = port;
	...
	public void run() {
		...
		try {
		    ...
		    System.out.println("AbstractBlockingClient starts running [thread:" + Thread.currentThread().getId() + "]");
			...
			while (state.get() == State.RUNNING) { 
				...
				if (limit>=byteLength) {
					int messageLen;
					do {
						// 1), if data package is big (@messageLen), cache the data into inBuffer until 
						// the whole package (multiple requests) is received
						// 2), if cached data is bigger than @defaultBUfferSize, replace inBuffer with
						// a new bigger buffer
						// 3), if a package is splited into multiple requests, follow 1)
					...
					} while (messageLen<limit); // when is this condition true??
				}
			}
